...
from account import ACCOUNT_CACHE_TIMEOUT, ACCOUNT_CACHE_KEY

class AccountManager(models.Manager):
    def get_for_user(self, user):
        account = cache.get('%s_%s' % (ACCOUNT_CACHE_KEY, user.pk))
        if not account:
            account = self.get(user=user)
            cache.set('%s_%s' % (ACCOUNT_CACHE_KEY, user.pk), account, ACCOUNT_CACHE_TIMEOUT)
        return account
...
def update_cache(sender, instance, **kwargs):
    cache.set('%s_%d' % (ACCOUNT_CACHE_KEY, instance.user_id), instance, ACCOUNT_CACHE_TIMEOUT)

@receiver(models.signals.post_delete, sender=Account)
def delete_cache(sender, instance, **kwargs):
    cache.delete('%s_%d' % (ACCOUNT_CACHE_KEY, instance.user_id))
