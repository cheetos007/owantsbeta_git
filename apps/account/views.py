...
from invite_friends.helpers import get_invite_parameters, accept_invite
...
def login(request, **kwargs):
    ...
    if request.method == "POST" and not url_required:
        ...
        form = form_class(group=group, initial={'email': ugettext("Username/email")})
...
def signup(request, **kwargs):
    ...
    if request.method == "POST":
        ...
        form = form_class(group=group, initial={'username': ugettext('Username'), 'email': ugettext('Email address')})
...
def login_error(request):
    return render_to_response("account/login_error.html", RequestContext(request))
